...
$global:RomExtensions = @(".zip",".7z",".iso",".chd",".bin",".cue",".dosz",".gba",".gbc",".gb",".nes",".sfc",".smc",".n64",".pbp",".nds",".vmu",".img",".fds",".md",".gen",".p8",".sh",".png",".ldb",".mgw")
$global:SourcePath    = ""
$global:DestPath      = ""
$global:AbortSync     = $false

# --- 1.3: MASTER SYSTEM DATABASE (Rows 1-135) ---
...
[PSCustomObject]@{ID=55    ;Name="Mame 2010"               ;Folder="mame"             ;Alias="---"         ;G350_Src="---"              ;ArkOS=$true    ;dArkOS=$true    ;RE_ID="55"},     
...
# --- 1.4: DATABASE OS-SPECIFIC FLAG LOGIC ---
# --- [PSCustomObject]@{ID=xx.1 = dArkOS_RE_ID specific folders
# --- [PSCustomObject]@{ID=xx.2 = G350_Src specific folders
# --- 1.5: DATABASE SPECIAL ADDITIONS ---
...
# --- 2.1.1: HEADER & TITLE ---
# This automatically grabs "v2.2b" (or whatever you named the file)
$scriptTitle = (Get-Item $PSCommandPath).BaseName
...
# --- 3.5: CLONE MODE GROUPBOX ---
$global:gbCloneMode = New-Object Windows.Forms.GroupBox
$global:gbCloneMode.Text     = "Clone Mode"
$global:gbCloneMode.Location = New-Object System.Drawing.Point(590, 321)
$global:gbCloneMode.Size     = New-Object System.Drawing.Size(180, 60)

$global:rbFoldersOnly = New-Object Windows.Forms.RadioButton
$global:rbFoldersOnly.Text     = "Folders Only"
$global:rbFoldersOnly.Location = New-Object System.Drawing.Point(10, 13)
...
$global:rbEverything.Location = New-Object System.Drawing.Point(10, 33)
$global:gbCloneMode.Controls.AddRange(@($global:rbFoldersOnly, $global:rbEverything))

$global:btnClone = New-Object Windows.Forms.Button
$global:btnClone.Text     = "POWER CLONE"
$global:btnClone.Location = New-Object System.Drawing.Point(30, 325)
$global:btnClone.Size     = New-Object System.Drawing.Size(550, 55)
$global:btnClone.BackColor = [System.Drawing.ColorTranslator]::FromHtml("#ffd700")
$global:btnClone.Font      = New-Object Drawing.Font("Segoe UI", 9, [Drawing.FontStyle]::Bold)
...
# Function 4.7: Centralized File Transfer Logic (v2.0b High-Speed Stream)
function Start-FileTransfer {
    ...
    $global:lblAction.Visible = $true
    $lastFolder = "" # Track folder changes
    ...
    foreach ($file in $processedFiles) {
        ...
        $currentFolder = Split-Path $file.Dir -Leaf # Get current folder name

        # --- LOGGING SURGERY: FOLDER LEVEL ONLY ---
        if ($currentFolder -ne $lastFolder) {
            # Find ID and Name from DB to match the OS CREATED format
            $dbMatch = $global:SystemDatabase | Where-Object { $_.Folder -eq $currentFolder }
            $id   = if($dbMatch){ $dbMatch.ID }   else { "---" }
            $name = if($dbMatch){ $dbMatch.Name } else { $currentFolder }
            
            # Report the SYSTEM to the log window
        ...
        if (!(Test-Path $targetFile) -or ($file.LastWrite -gt (Get-Item $targetFile).LastWriteTime)) {
            ...
            robocopy $file.Dir $targetDir $file.Name /Z /MT:16 /R:1 /W:1 /NP /NFL /NDL /NJH /NJS | Out-Null
            $count++
        }

        # --- PROGRESS & DASHBOARD: FILE LEVEL ---
        $bytesDone += $file.Size
        Update-UIProgress -BytesDone $bytesDone -TotalBytes $totalBytes -Stopwatch $sw -FileName $file.Name
        
        # --- THE MISSING LINK: LET THE UI BREATHE ---
        [System.Windows.Forms.Application]::DoEvents() # <--- ADD THIS HERE
    }
    ...
    if ($global:AbortSync) {
        Update-Log "!!! SYNC/CLONE HALTED BY USER !!!" $true
        $global:lblAction.Text = "ABORTED: Process Incomplete."
        $global:lblAction.ForeColor = [System.Drawing.Color]::Red
    } 
    else {
        Update-Log "SUCCESS: Operation Finished." $true
        $global:lblAction.Text = "Operation Complete."
        # Audit only runs on Success
        Run-Audit $global:DestPath "Target_Audit.txt"
    }
}

# Function 4.8: Smart Sync & Brute Clone Handlers
function Invoke-UniversalSync {
    ...
    # 2. Discovery with Filter (Uses global:RomExtensions)
...
function Invoke-PowerClone {
    ...
    if ($global:rbFoldersOnly.Checked) {
        ...
        # Robocopy /E /XF * creates directories but excludes all files
        ...
        # Auto-Audit the new structure
        Run-Audit $global:DestPath "Target_Audit.txt"
    }
    
    # --- CHOICE B: EVERYTHING (No Filtering) ---
    else {
        $global:Log.AppendText("`r`n" + ("=" * 60) + "`r`n")
        Update-Log "SCANNING ALL FILES (Brute Force Mirror)..." $true
        ...
        # Hand-off to Master Engine (Unfiltered)
...
function Show-HelpManual {
    ...
    $global:Log.AppendText("      ***** First select a MASTER and a TARGET folder to activate the functional bottons. *****`r`n`r`n" )
...
$global:btnClone.Add_Click({   Reset-ActionDisplay; Invoke-PowerClone })
...
$global:lblGitHub.add_LinkClicked({ 
    # 1. Grab "v2.2d" (or whatever the file is named)
